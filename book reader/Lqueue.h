/*****************************************
** 
** Created a templated Linked list that can hold any data type
*
** 
***********************************************/
...
Lqueue<T>::~Lqueue(){
  ...
  Lqueue<T>::Lqueue(const Lqueue& copyLqueue){
    ...
    m_head=nullptr;
    //sets currNode to be the copy's head
    Node<T>* currNode= copyLqueue.m_head;   
    T data;
    //Iterate over the copy node and set the new list to 
    // be the old list
  ...
  Lqueue<T>& Lqueue<T>::operator=(const Lqueue& copyLqueue){
    //Same deal as the copy constructor but return the list
    m_head=nullptr;
    Node<T>* currNode= copyLqueue.m_head;   
    T data;
    while(currNode!=nullptr){
  ...
  void Lqueue<T>::Push(const T& data, bool ifFront){
    ...
    if(m_head==nullptr){
      //make the new node the head and set the next one to null
        m_head=newNode;
        m_head->SetNext(nullptr);
        m_size=ZERO;
        m_tail=newNode;
      }
      //if it is pushed to be pushed to the back
    else if(ifFront){
      tempNode=m_head;
  ...
  void Lqueue<T>::Swap(int position){
    ...
    if(m_size==TWO){
      ...
      for(int i=ZERO;i<position-ONE;i++){
        prePreNode=prevNode;
      ...
      prevNode=m_head;
      lastNode=m_head->GetNext();
      m_head=lastNode;
      prevNode->SetNext(lastNode->GetNext());
      m_head->SetNext(prevNode);
    }
    else{
      for(int i=ZERO;i<position-ONE;i++){
        prePreNode=prevNode;
