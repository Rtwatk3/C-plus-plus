...
typedef std::pair<int,int> src_dest_t;//data

// Struct for data items on the stack
struct StackEntry { //The nodes
  src_dest_t src_dest;   // source-dest pair <--data
...
typedef struct StackEntry entry_t; //top?
...
public:
  ...
  //This is what we are using to solve the maze
